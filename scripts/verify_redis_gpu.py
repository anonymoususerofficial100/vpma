...
REDIS_PORT = 6379
REDIS_CA_CERT = "/home/user/redis_certs_test/ca.crt"
REDIS_CERT = "/home/user/redis_certs_test/redis.crt"
REDIS_KEY = "/home/user/redis_certs_test/redis.key"
...
def verify_anchor_signature(redis: RedisClient, tenant: str, pinned_pubkey: str = None):
    """Verify the enclave's PUBLIC anchor signature over (tenant, block, chained_root).

    This is the only part of the store a third party can check without holding a secret. The HMAC
    anchor at gpu_checkpoint:<tenant> cannot serve that purpose: whoever can verify it can also
    forge it, so it answers "did the enclave write this?" for the enclave itself and nothing more.

    ON THE PUBLIC KEY. The record carries the key it was signed with, which is convenient and NOT a
    trust root -- the host holds the Redis credentials and can replace both the key and the
    signature. Pass --pubkey with a value pinned out of band (the enclave prints it at startup) and
    this compares against it. Without one, the check degrades to "internally consistent", which is
    worth saying out loud rather than reporting as success.
    """
    line = redis.command("GET", f"gpu_anchor_sig:{tenant}")
    if not line or not isinstance(line, str) or not line.startswith("anchor-ecdsa-p256-v2|"):
        return None, "no public anchor signature stored for this tenant"

    try:
        _tag, t, block, root_hex, sig_hex, pub_hex = line.split("|")
    except ValueError:
        return False, "malformed anchor signature record"
    if t != tenant:
        return False, f"record is for tenant '{t}', not '{tenant}'"

    msg = (b"checkpoint-v1|" + tenant.encode() + b"|"
           + int(block).to_bytes(8, "little") + bytes.fromhex(root_hex))

    digest = sha256(b"VPMA-anchor-ecdsa-p256-v2\x00" + msg)
    ...
    try:
        ...
        return None, "python-cryptography not installed; cannot verify the anchor signature"

    if pinned_pubkey:
        if pinned_pubkey.strip().lower() != pub_hex.strip().lower():
            return False, ("PUBLIC KEY MISMATCH - the stored key is not the pinned one. Either the "
                           "enclave identity changed, or someone substituted the key. Do not trust "
                           "this signature.")
        trust = "pinned key"
    else:
        trust = "STORED key (NOT pinned - see --pubkey)"

    try:
        load_der_public_key(bytes.fromhex(pub_hex)).verify(
            bytes.fromhex(sig_hex), digest, ec.ECDSA(Prehashed(_h.SHA256())))
    except Exception as e:
        return False, f"signature INVALID: {e}"
    return True, f"block {block}, root {root_hex[:16]}..., verified against the {trust}"

def verify_chain(redis: RedisClient, tenant: str = None,
                 since: int = 0) -> Tuple[bool, List[Dict[str, Any]]]:
    """Verify the chained roots, PER TENANT.

    Blocks from every node share one Redis keyspace -- the tenant is only a hash field, never part
    of the key -- and block ids come from a single global INCR, so a host and a guest writing
    concurrently interleave their ids. Walking all block:* with one zero-seeded chain therefore
    breaks at the second tenant's genesis (prev == zeros where the walker expected the previous
    tenant's root) and reports INVALID for data that is perfectly well formed. Each tenant owns an
    independent chain and must be walked independently.
    """
    results = []

    by_tenant: Dict[str, List[str]] = {}
    for k in redis.keys("block:*"):
        try:
            bid = int(k.split(':')[1])
        ...
        if bid < since:
            continue
        owner = (redis.hgetall(k) or {}).get('vm_name', '') or '(unnamed)'
        if tenant is not None and owner != tenant:
            continue
        by_tenant.setdefault(owner, []).append(k)

    overall_ok = True
    for owner in sorted(by_tenant):
        block_keys = sorted(by_tenant[owner], key=lambda k: int(k.split(':')[1]))

        prev_chained_root = None
        n_ok = 0
        first_bad = None
        ...
        for block_key in block_keys:
            ...
            if prev_chained_root is None:
                prev_chained_root = stored_prev
            if stored_prev != prev_chained_root:
                ...
                overall_ok = False
                first_bad = block_id if first_bad is None else first_bad
                break
            ...
            try:
                ...
                results.append({
                    ...
                    "block_id": block_id,
                    "chain_valid": False,
                    "error": "malformed hex in prev_chained_root/merkle_root",
                })
                overall_ok = False
                first_bad = block_id if first_bad is None else first_bad
                break
        ...
        results.append({
            "tenant": owner,
            "summary": True,
            "blocks_in_scope": len(block_keys),
            "verified": n_ok,
...
def verify_chain_to_block(redis: RedisClient, target_block: int) -> Tuple[bool, Dict[str, Any]]:
    ...
    target_key = f"block:{target_block}"
    target_tenant = (redis.hgetall(target_key) or {}).get('vm_name', '') or '(unnamed)'
    ...
    for k in redis.keys("block:*"):
        ...
        if bid > target_block:
            continue
        tenant = (redis.hgetall(k) or {}).get('vm_name', '') or '(unnamed)'
        if tenant == target_tenant:
            block_keys.append(k)
    block_keys.sort(key=lambda k: int(k.split(':')[1]))
...
def main():
    ...
    parser.add_argument("--tenant", default=None,
                       help="Restrict --chain to one tenant (vm_name). Tenants share one Redis "
                            "keyspace and one global block-id counter, so their chains interleave.")
    parser.add_argument("--pubkey", default=None,
                       help="Hex DER public key to PIN the anchor signature against. The enclave "
                            "prints it at startup; record it out of band. Without this the anchor "
                            "is only checked for internal consistency, since the host can rewrite "
                            "the stored key.")
    parser.add_argument("--since", type=int, default=0,
                       help="Restrict --chain to blocks with id >= SINCE, e.g. to audit one run "
                            "without being stopped by older blocks in a superseded format.")
    ...
    try:
        if args.chain:
            ...
            is_valid, results = verify_chain(redis, tenant=args.tenant, since=args.since)
            chain_time = (time.time() - chain_start) * 1000

            for r in results:
                if r.get('summary'):
                    verified = r['verified']
                    total = r['blocks_in_scope']
                    fb = r['first_failure']
                    mark = "OK" if fb is None else "FAIL"
                    detail = "all verified" if fb is None else f"first failure at block {fb}"
                    print(f"    [{mark}] {r['tenant']}: {verified}/{total} blocks verified - {detail}")
                    continue
                status = "OK" if r.get('chain_valid') else "FAIL"
                if r.get('error'):
                    print(f"      block {r['block_id']}: [{status}] {r['error']}")
                else:
                    print(f"      block {r['block_id']}: [{status}] "
                          f"merkle={r.get('merkle_root', 'N/A')} "
                          f"chained={r.get('chained_root', 'N/A')}")

            for owner in sorted({r.get('tenant') for r in results if r.get('tenant')}):
                ok, detail = verify_anchor_signature(redis, owner, args.pubkey)
                mark = "OK" if ok else ("-" if ok is None else "FAIL")
                print(f"    [{mark}] anchor signature ({owner}): {detail}")
                if ok is False:
                    is_valid = False

            print(f"\n[{'OK' if is_valid else 'FAIL'}] Chain verification: {'VALID' if is_valid else 'INVALID'}")
            if not is_valid:
                print("    Note: each tenant's chain is independent. A tenant failing at its OLDEST")
                print("    block is usually data written before the enclave adopted DOMAIN_CHAIN,")
                print("    not tampering - scope the audit with --tenant NAME --since BLOCK_ID.")
