#include <uapi/linux/ptrace.h>
#include <linux/sched.h>

#define MAX_GPU_ENTRIES 64

struct gpu_reading {
    u64 energy_uj;
    u64 timestamp_ns;
    u32 gpu_index;
    u32 domain_id;
    u64 hash;
    u8  valid;
    u8  padding[7];
};
BPF_ARRAY(gpu_hash_map, struct gpu_reading, MAX_GPU_ENTRIES);

BPF_HASH(gpu_ctx_hint, u64, u32);

BPF_HASH(gpu_energy_scratch, u64, u64);

#define TAG_PRODUCER_GPU 0x8000u

#define SIPTAG_PRODUCER_GPU_NVML 3u

#define SIPTAG_K0 0xSIPTAG_K0_PLACEHOLDERULL
#define SIPTAG_K1 0xSIPTAG_K1_PLACEHOLDERULL
#define SIPTAG_EPOCH SIPTAG_EPOCH_PLACEHOLDERu

#define SIPTAG_VERSION 1u

#define SIPTAG_LEN_BLOCK 0x2000000000000000ULL

static __always_inline u64 siptag_rotl(u64 x, int b) {
    return (x << b) | (x >> (64 - b));
}

#define SIPROUND(v0, v1, v2, v3) do {                       \
    v0 += v1; v1 = siptag_rotl(v1, 13); v1 ^= v0; v0 = siptag_rotl(v0, 32); \
    v2 += v3; v3 = siptag_rotl(v3, 16); v3 ^= v2;           \
    v0 += v3; v3 = siptag_rotl(v3, 21); v3 ^= v0;           \
    v2 += v1; v1 = siptag_rotl(v1, 17); v1 ^= v2; v2 = siptag_rotl(v2, 32); \
} while (0)

#define SIPABSORB(v0, v1, v2, v3, m) do {                   \
    v3 ^= (m); SIPROUND(v0, v1, v2, v3); SIPROUND(v0, v1, v2, v3); v0 ^= (m); \
} while (0)

static __always_inline u64 siptag(u64 energy_uj, u64 timestamp_ns,
                                  u32 unit_id, u32 domain_id,
                                  u16 version, u16 producer, u32 key_epoch) {
    u64 v0 = 0x736f6d6570736575ULL ^ SIPTAG_K0;
    u64 v1 = 0x646f72616e646f6dULL ^ SIPTAG_K1;
    u64 v2 = 0x6c7967656e657261ULL ^ SIPTAG_K0;
    u64 v3 = 0x7465646279746573ULL ^ SIPTAG_K1;

    u64 m2 = ((u64)unit_id) | (((u64)domain_id) << 32);
    u64 m3 = ((u64)version) | (((u64)producer) << 16) | (((u64)key_epoch) << 32);

    SIPABSORB(v0, v1, v2, v3, energy_uj);
    SIPABSORB(v0, v1, v2, v3, timestamp_ns);
    SIPABSORB(v0, v1, v2, v3, m2);
    SIPABSORB(v0, v1, v2, v3, m3);
    SIPABSORB(v0, v1, v2, v3, SIPTAG_LEN_BLOCK);

    v2 ^= 0xffULL;
    SIPROUND(v0, v1, v2, v3);
    SIPROUND(v0, v1, v2, v3);
    SIPROUND(v0, v1, v2, v3);
    SIPROUND(v0, v1, v2, v3);
    return v0 ^ v1 ^ v2 ^ v3;
}

int trace_nvml_energy_entry(struct pt_regs *ctx) {
    u64 energy_ptr = (u64)PT_REGS_PARM2(ctx);
    if (energy_ptr == 0)
        return 0;
    u64 tgid = bpf_get_current_pid_tgid() >> 32;
    gpu_energy_scratch.update(&tgid, &energy_ptr);
    return 0;
}

int trace_nvml_energy_ret(struct pt_regs *ctx) {
    u64 tgid = bpf_get_current_pid_tgid() >> 32;
    u64 *energy_ptr = gpu_energy_scratch.lookup(&tgid);
    if (!energy_ptr) {

        gpu_ctx_hint.delete(&tgid);
        return 0;
    }

    if (PT_REGS_RC(ctx) == 0) {
        u32 *gpu_idx = gpu_ctx_hint.lookup(&tgid);
        if (gpu_idx) {
            u32 idx = *gpu_idx;
            if (idx < MAX_GPU_ENTRIES) {
                u64 energy_mj = 0;
                long rc = bpf_probe_read_user(&energy_mj, sizeof(energy_mj), (void *)*energy_ptr);
                if (rc == 0) {
                    u64 energy_uj = energy_mj * 1000;
                    struct gpu_reading *r = gpu_hash_map.lookup(&idx);
                    if (r) {
                        r->energy_uj = energy_uj;
                        r->timestamp_ns = bpf_ktime_get_ns();
                        r->gpu_index = idx;

                        r->domain_id = TAG_PRODUCER_GPU;
                        r->hash = siptag(energy_uj, r->timestamp_ns, idx, TAG_PRODUCER_GPU,
                                         SIPTAG_VERSION, SIPTAG_PRODUCER_GPU_NVML, SIPTAG_EPOCH);
                        r->valid = 1;
                    }
                }
            }
        }
    }

    gpu_ctx_hint.delete(&tgid);
    gpu_energy_scratch.delete(&tgid);
    return 0;
}

struct gpu_access_event {
    u64 ts_ns;
    u32 pid;
    u32 uid;
    u64 cgroup_id;
    u32 cmd;
    char comm[16];
};
BPF_PERF_OUTPUT(gpu_access_events);

int trace_nvidia_ioctl(struct pt_regs *ctx) {
    struct gpu_access_event ev = {};
    ev.ts_ns = bpf_ktime_get_ns();
    ev.pid = bpf_get_current_pid_tgid() >> 32;
    ev.uid = bpf_get_current_uid_gid() & 0xffffffff;
    ev.cgroup_id = bpf_get_current_cgroup_id();
    ev.cmd = (u32)PT_REGS_PARM2(ctx);
    bpf_get_current_comm(&ev.comm, sizeof(ev.comm));
    gpu_access_events.perf_submit(ctx, &ev, sizeof(ev));
    return 0;
}
